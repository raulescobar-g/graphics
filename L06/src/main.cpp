...
static void mouse_callback(GLFWwindow *window, int button, int action, int mods)
{
	...
	if(action == GLFW_PRESS) {
		...
		newPt[0] = sx * posX + xMinWorld;
		newPt[1] = sy * posY + yMinWorld;
...
static void init()
{
	...
	rainbowID = glGetAttribLocation(progID, "rainbow");

	dotyID = glGetUniformLocation(progID, "doty");

	dotxID = glGetUniformLocation(progID, "dotx");
	...
	colorBuf.push_back( 0.0f); // x
	colorBuf.push_back(1.0f); // y
	colorBuf.push_back( 0.0f); // z

	colorBuf.push_back( 0.0f); // x
	colorBuf.push_back( 0.0f); // y
	colorBuf.push_back( 1.0f); // z


	colorBuf.push_back(1.0f); // x
	colorBuf.push_back(0.0f); // y
	colorBuf.push_back( 0.0f); // z
	...
	posBuf.push_back(-1.2f); // x
	posBuf.push_back(-1.0f); // y
	posBuf.push_back( 0.0f); // z

	posBuf.push_back( -0.2f); // x
	posBuf.push_back(1.0f); // y
	posBuf.push_back( 0.0f); // z
...
static void render()
{
	...
	int dot_y = (height/4.0) * sin(t) + (height/2.0);
	int dot_x = (width/4.0) * cos(t) + (width/2.0);
	...
	glUniform1i(dotyID, dot_y);
	glUniform1i(dotxID, dot_x);
	// Enable the attribute
	glEnableVertexAttribArray(vertPosAttrID);
	glEnableVertexAttribArray(rainbowID);
	...
	glBindBuffer(GL_ARRAY_BUFFER, colBufID);
	glVertexAttribPointer(rainbowID, 3, GL_FLOAT, GL_FALSE,0,(void *)0 );
	// Actually draw here
	...
	glDisableVertexAttribArray(rainbowID);
